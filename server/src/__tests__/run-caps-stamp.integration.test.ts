// Layer-1 integration proof for Combo-01 Phase 2a per-run caps: a queued
// heartbeat run claimed via heartbeat.startNextQueuedRunForAgent lands in the
// `running` status with the company's resolved wall-clock/cost ceilings
...
// __tests__/run-wind-down.integration.test.ts. Only the company-cap seed,
// claim, and stamped-cap assertions are new.
...
const mockAdapterExecute = vi.hoisted(() =>
  vi.fn(async () => ({
    ...
    timedOut: false,
    errorMessage: null,
    summary: "Run-caps stamp integration test run.",
    provider: "test",
    model: "test-model",
  })),
);
...
if (!embeddedPostgresSupport.supported) {
  console.warn(
    `Skipping embedded Postgres run-caps stamp tests on this host: ${embeddedPostgresSupport.reason ?? "unsupported environment"}`,
  );
}

describeEmbeddedPostgres("claimQueuedRun stamps per-run caps (integration)", () => {
  let db!: ReturnType<typeof createDb>;
  let heartbeat!: ReturnType<typeof heartbeatService>;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("paperclip-run-caps-stamp-");
  ...
  it("stamps the resolved company cap onto the run at claim", async () => {
    ...
    await db
      .update(companies)
      .set({ maxRunCostCents: 250, maxRunWallClockMs: 600000 })
